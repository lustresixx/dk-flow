...
- **模板页**：7 个内置模板（结构预览），可「直接运行」或「创建并编排」；创建时不填参数，留空的必填参数会自动转为运行时询问字段
- **工作流页**：项目/个人实例列表，运行 / 编排 / 删除；运行参数输入框放在这里——启动前可预填，留空则在运行时逐个询问
- **运行记录页**：**运行时拓扑图**（状态按成功/失败着色、当前状态高亮、执行过的转移路径加亮）+ 状态时间线、每步输出与 verdict、重试次数、supervisor 评分、恢复 / 停止
...
**复用到你自己的项目**：创建时可以不填参数——留空的 `projectRoot`（项目绝对路径）与 `requirements`（优化目标与验收期望）会在每次启动工作流时询问；也可以现在预填固化。工作台「模板」页选「代码优化评审」直接运行（弹窗补全参数）；或对话中说：
